...
open class SplitTabBarMasterController: UITableViewController {
        ...
        -> UITableViewCell
    {
        ...
        cell.textLabel?.text =
            ...
            ?? (self.splitViewController as? SplitTabBarViewController)?.detailTabBar
            .viewControllers?[indexPath.row].title

        if let row = (self.splitViewController as? SplitTabBarViewController)?.detailTabBar
            .selectedViewControllerIndex
        {
            ...
            if indexPath == selectedIndexPath {
                cell.imageView?.image = (self.splitViewController as? SplitTabBarViewController)?
                    .detailTabBar.viewControllers?[indexPath.row].tabBarItem.selectedImage?
                    .withRenderingMode(.alwaysTemplate)
            } else {
                cell.imageView?.image = (self.splitViewController as? SplitTabBarViewController)?
                    .detailTabBar.viewControllers?[indexPath.row].tabBarItem.image?
                    .withRenderingMode(.alwaysTemplate)
            }
        }
    ...
    override public func viewDidAppear(_ animated: Bool) {
        ...
        guard
            let index = (self.splitViewController as? SplitTabBarViewController)?.detailTabBar
                .selectedViewControllerIndex
        else { return }
